...
# opencv의 pre trained model을 통해서, 예측하기위해서는
...
# 각 숫자에 매핑되는 색깔로 셋팅해 준 것이다.
# 따라서 각 픽셀별 색깔 정보가 들어가게 되었다.
# 2차원 행렬을, 3차원 채널이 있는 RGB행렬로 만든다.
mask_class_map = CV_ENET_SHAPE_IMG_COLORS[class_map]

# 리사이즈 한다.
# 인터폴레이션을 ITNER_NEAREST로 한 이유는 ?
...
for ( i, (class_name, img_color)) in enumerate( zip( label_values, CV_ENET_SHAPE_IMG_COLORS) ) :
  ...
  cv2.putText(my_legend, class_name, (5, (i*25) + 17), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2 )
